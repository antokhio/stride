...
/// </summary>
/// <remarks>
/// The <c>.slnx</c> format (introduced in Visual Studio 2022 17.10+) is an XML document whose root
/// <c>Solution</c> element contains <c>Project</c> and <c>Folder</c> elements. Unlike the legacy
/// <c>.sln</c> text format, projects do not carry a GUID and their type is usually inferred from the
/// file extension. This reader only resolves the information required by the engine and tooling
/// (project paths, inferred type and folder hierarchy); build configuration mappings are ignored.
/// </remarks>
internal static class SlnxReader
{
    ...
    public static Solution ReadSolutionFile(string solutionFullPath, Stream stream)
    {
        ...
        {
            throw new SolutionFileException($"Invalid .slnx file '{solutionFullPath}': the file is not a well-formed XML document.", ex);
        }

        var root = document.Root
            ?? throw new SolutionFileException($"Invalid .slnx file '{solutionFullPath}': the document is empty.");

        if (!string.Equals(root.Name.LocalName, "Solution", StringComparison.Ordinal))
        {
            throw new SolutionFileException($"Invalid .slnx file '{solutionFullPath}': expected root element 'Solution' but found '{root.Name.LocalName}'.");
        }
    ...
    private static void ReadContainer(XElement container, Guid parentGuid, Solution solution, string solutionDirectory)
    {
    ...
    private static Project ReadProject(XElement element, Guid parentGuid, string solutionDirectory)
    {
        var path = (string?)element.Attribute("Path")
            ?? throw new SolutionFileException("Invalid .slnx file: a 'Project' element is missing the required 'Path' attribute.");
    ...
    {
        var folderPath = (string?)element.Attribute("Name")
            ?? throw new SolutionFileException("Invalid .slnx file: a 'Folder' element is missing the required 'Name' attribute.");
